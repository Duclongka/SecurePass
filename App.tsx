...
const App: React.FC = () => {
  ...
  const handleLock = () => {
    ...
    setMasterPassword('');
    setView('login');
    setUploadedKeyFile(null); // Reset uploaded file state
    ...
    try {
      if (isVerifyingImport && pendingImportData) {
        ...
        if (result.success) {
          ...
          const encrypted = await SecurityService.encryptVault(JSON.stringify(result.data), passToUse);
  ...
  const handleKeyFileSelection = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Check filename format: must start with "Key_Secure"
    if (!file.name.startsWith('Key_Secure')) {
      setToast('File không hợp lệ (Sai định dạng tên)');
        ...
        <form onSubmit={onUnlockClick} className="space-y-4">
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              {loginError && (
                <p className="text-[10px] text-red-500 font-bold text-center">Mật khẩu hoặc File Khóa không chính xác</p>
              )}
            </div>
          </div>
    ...
    <div onClick={() => setSelectedEntry(entry)} className={`border rounded-2xl p-4 flex items-center justify-between transition-all cursor-pointer ${isDark ? 'bg-[#181818] border-white/5' : 'bg-white border-gray-200 shadow-sm'}`}>
      <div className="flex items-center gap-3 overflow-hidden">
        ...
        <div className="overflow-hidden">
          ...
          <p className="text-[10px] tracking-tighter mt-1 text-gray-500 truncate lowercase">{getSubText()}</p>
        </div>
      </div>
...
const MasterPasswordModal = ({ t, isDark, onClose, setMasterPassword, settings, setSettings, masterPassword: currentMP }: any) => {
  ...
  const togglePass = (field: string) => setShowPass(p => ({ ...p, [field]: !p[field] }));

  const handleVerifyOld = async () => {
    setIsProcessing(true);
    setError(null);
    try {
      const keyFileStr = localStorage.getItem('securepass_master_hash');
      if (!keyFileStr) {
        // If no key file, we can't verify old password properly in 2FA mode
        // But if we are here, we might be in a fallback state
        if (oldMP === currentMP) setStep(2);
        else setError(t.wrongPassword);
        return;
      }
      const result = await SecurityService.verifyAccess(oldMP, keyFileStr);
      if (result.success) {
        setStep(2);
      } else {
  ...
  const handleSaveNew = async () => {
    ...
    try {
      if (currentMP) {
        // CHANGE MODE: Use reKeySystem
        const encryptedVault = localStorage.getItem('securepass_vault');
        const vaultData = encryptedVault ? JSON.parse(encryptedVault) : null;
        
        const result = await SecurityService.reKeySystem(oldMP || currentMP, newMP, vaultData);
        if (result.success) {
          setKeyFile(result.newKeyFile);
          localStorage.setItem('securepass_vault', JSON.stringify(result.newVault));
          localStorage.setItem('securepass_master_hash', JSON.stringify(result.newKeyFile));
          localStorage.setItem('securepass_remembered_mp', newMP);
          setMasterPassword(newMP, result.newKeyFile);
          setStep(3);
        } else {
          setError(result.error === 'ERR_INVALID_PASSWORD' ? t.wrongPassword : "Lỗi thay đổi mật khẩu");
        }
      } else {
        // CREATE MODE
        const { salt, authHash, iterations } = await SecurityService.initMasterAuth(newMP);
        const kf = {
          type: 'MASTER_KEY_FILE',
          version: '4.0.0',
          salt,
  ...
  const downloadKeyFile = async () => {
    ...
    if (available) {
      ...
      onClose();
      if (currentMP) {
        alert("Mật khẩu đã được đổi. Vui lòng tạo bản sao lưu dữ liệu mới để đảm bảo an toàn.");
      }
    }
  };
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[120] flex items-center justify-center p-6 animate-in fade-in">
      <div className={`w-full max-w-sm rounded-[2.5rem] border p-8 space-y-6 text-center ${isDark ? 'bg-[#121212] border-white/10' : 'bg-white border-black/5'}`}>
        ...
        <div className="space-y-3">
          ...
          <button onClick={() => { onClose(); if(currentMP) alert("Mật khẩu đã được đổi. Vui lòng tạo bản sao lưu dữ liệu mới để đảm bảo an toàn."); }} className={`w-full py-4 rounded-3xl font-bold uppercase tracking-widest text-xs border transition-all ${isDark ? 'bg-white/5 border-white/5 text-gray-400' : 'bg-gray-50 border-gray-200 text-black'}`}>
            Để sau
          </button>
        </div>
      </div>
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[120] flex items-center justify-center p-6 animate-in fade-in">
      <div className={`w-full max-w-sm rounded-[2.5rem] border p-8 space-y-6 text-center ${isDark ? 'bg-[#121212] border-white/10' : 'bg-white border-black/5'}`}>
        ...
        <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-2xl">
          <p className="text-[10px] text-yellow-600 font-bold uppercase leading-tight">Cảnh báo: Các bản sao lưu cũ vẫn yêu cầu mật khẩu cũ. Hãy tạo bản sao lưu mới ngay sau khi lưu file khóa này.</p>
        </div>
        <button onClick={downloadKeyFile} className="w-full bg-[#4CAF50] text-white py-4 rounded-3xl font-bold uppercase tracking-widest text-xs shadow-lg shadow-[#4CAF50]/20 flex items-center justify-center gap-2">
          <Icons.Download size={16}/> {t.saveKeyFileBtn}
        </button>
      </div>
    </div>
  );
      ...
      <div className={`w-full max-w-md rounded-[2.5rem] border p-8 space-y-6 shadow-2xl ${isDark ? 'bg-[#121212] border-white/10' : 'bg-white border-black/5'}`}>
        <div className="flex justify-between items-center">
          <h2 className={`text-xl font-black ${!isDark ? 'text-black' : ''}`}>
            {currentMP ? "Đổi mật khẩu chủ" : t.createMasterPass}
          </h2>
        ...
        {step === 1 ? (
          <div className="space-y-4 animate-in slide-in-from-right">
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase text-gray-500 ml-1">Nhập mật khẩu hiện tại</label>
              <div className="relative">
                <input 
                  type={showPass.old ? "text" : "password"} 
                  value={oldMP} 
                  onChange={e => setOldMP(e.target.value)} 
                  className={`w-full border rounded-2xl py-4 px-6 pr-12 outline-none transition-all text-base ${isDark ? 'bg-[#1a1a1a] border-white/5 text-white focus:border-[#4CAF50]/50' : 'bg-gray-100 border-gray-200 text-black focus:border-[#4CAF50]/50'}`} 
                />
                <button type="button" onClick={() => togglePass('old')} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500">
                  {showPass.old ? <Icons.EyeOff size={18}/> : <Icons.Eye size={18}/>}
                </button>
              </div>
            ...
            <button 
              ...
              disabled={!oldMP || isProcessing} 
              className="w-full bg-[#4CAF50] text-white py-4 rounded-3xl font-bold uppercase tracking-widest text-xs disabled:opacity-30 active:scale-95 transition-all shadow-lg shadow-[#4CAF50]/20 flex items-center justify-center gap-2"
            >
              {isProcessing ? <Icons.Loader2 className="animate-spin" size={16}/> : <Icons.ArrowRight size={16}/>}
              Tiếp tục
            </button>
          </div>
        ) : (
          <div className="space-y-4 animate-in slide-in-from-right">
            <div className="space-y-1">
              ...
              <div className="relative">
                <input 
                  ...
                  className={`w-full border rounded-2xl py-4 px-6 pr-12 outline-none transition-all text-base ${isDark ? 'bg-[#1a1a1a] border-white/5 text-white focus:border-[#4CAF50]/50' : 'bg-gray-100 border-gray-200 text-black focus:border-[#4CAF50]/50'}`} 
                />
                <button type="button" onClick={() => togglePass('new')} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500">
                  {showPass.new ? <Icons.EyeOff size={18}/> : <Icons.Eye size={18}/>}
                </button>
              </div>
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                <input 
                  ...
                  className={`w-full border rounded-2xl py-4 px-6 pr-12 outline-none transition-all text-base ${isDark ? 'bg-[#1a1a1a] border-white/5 text-white focus:border-[#4CAF50]/50' : 'bg-gray-100 border-gray-200 text-black focus:border-[#4CAF50]/50'}`} 
                />
                <button type="button" onClick={() => togglePass('confirm')} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500">
                  {showPass.confirm ? <Icons.EyeOff size={18}/> : <Icons.Eye size={18}/>}
                </button>
              </div>
            ...
            <button 
              ...
              disabled={!isValid || newMP !== confirmMP || isProcessing} 
              className="w-full bg-[#4CAF50] text-white py-4 rounded-3xl font-bold uppercase tracking-widest text-xs disabled:opacity-30 active:scale-95 transition-all shadow-lg shadow-[#4CAF50]/20 flex items-center justify-center gap-2"
            >
              {isProcessing ? <Icons.Loader2 className="animate-spin" size={16}/> : <Icons.Save size={16}/>}
              {currentMP ? "Cập nhật mật khẩu" : t.save}
            </button>
          </div>
        )}
      </div>
  ...
  const renderMainList = () => (
    <div className="space-y-4 animate-in fade-in">
      <section className={`rounded-[2.5rem] p-8 border text-center ${isDark ? 'bg-[#161616] border-white/5' : 'bg-white border-gray-200 shadow-lg'}`}>
        ...
        <p className="text-[9px] font-bold text-gray-400 uppercase mt-1">{t.version}</p>
        <div className="h-px w-full my-4 bg-white/5" />
        <p className="text-[10px] font-bold text-gray-500 uppercase">by Loong Lee</p>
...
const EntryModal = ({ t, isDark, settings, mode, entry, onClose, onSave, copy, addType, setIsEditing, deleteEntry, deleteClickCount, setToast }: any) => {
  const [localData, setLocalData] = useState<any>(() => {
    if (entry) return { ...entry };
    const base = { type: addType || 'login', group: '---', subGroup: '', username: '', password: '', notes: '', atmPin: '', cardType: '', expiryMonth: '', cardNumber: '', cardHolder: '', frontImage: '', backImage: '', fullName: '', phone: '', email: '', postCode: '', address: '', documentType: '', idNumber: '', dob: '', gender: '', residence: '', birthRegPlace: '', hospital: '', issuer: '', issueDate: '', expiryDate: '', nationality: '', passportType: '', passportCode: '', class: '', msAuthCode: '', googleAuthQr: '', msAuthQr: '', customerSince: '', cvv: '', issuePlace: '' };
    if (addType === 'document') base.nationality = 'Việt Nam';
    return base;
  });
  ...
  const [ocrModal, setOcrModal] = useState<{type: 'card' | 'document', field: string} | null>(null);
  const [isOcrProcessing, setIsOcrProcessing] = useState(false);

  const handleOcrScan = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || !ocrModal) return;
    ...
    try {
      ...
      reader.onload = async (event) => {
        ...
        const result = await ocrService.scanImage(imageSrc, ocrModal.type);
        
        const newData = { ...localData };
        if (ocrModal.type === 'card') {
          if (result.cardNumber) newData.cardNumber = result.cardNumber;
          if (result.expiryDate) newData.expiryMonth = result.expiryDate;
        } else if (ocrModal.type === 'document') {
          if (result.idNumber) newData.idNumber = result.idNumber;
        }
      ...
      setToast("Lỗi nhận diện hình ảnh");
    } finally {
      setIsOcrProcessing(false);
    }
  };

  const copyableField = (label: string, field: string, value: string, type: string = "text", placeholder: string = "", showCopy: boolean = true, onScan?: () => void, forceUpper: boolean = false) => {
      ...
      <div className="space-y-1 w-full min-w-0 overflow-hidden">
        ...
        <div className="relative w-full">
          ...
          <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-2">
            {onScan && !isView && (
              <button type="button" onClick={onScan} className="text-gray-400 hover:text-[#4CAF50] p-2 transition-colors">
                <Icons.Camera size={16}/>
              </button>
            )}
            {showCopy && value && (
              <button type="button" onClick={() => copy(value)} className="text-gray-400 hover:text-[#4CAF50] p-2 transition-colors">
                <Icons.Copy size={14}/>
              </button>
            )}
            {isPassword && (
              <button type="button" onClick={() => setVisibleFields(prev => ({...prev, [field]: !prev[field]}))} className="text-gray-400 hover:text-[#4CAF50] p-2 transition-colors ml-1">
                {isVisible ? <Icons.EyeOff size={16}/> : <Icons.Eye size={16}/>}
              </button>
            )}
          </div>
        </div>
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex flex-col h-[100dvh] transition-colors ${isDark ? 'bg-[#0d0d0d]' : 'bg-[#f5f5f5]'}`}>
        ...
        <div className="max-w-md mx-auto space-y-5">
            ...
            <>
              ...
              {(advancedOpen || isView) && (
                <div className="space-y-5 animate-in slide-in-from-top-2">
                  ...
                  {copyableField(t.recoveryInfo, 'recoveryInfo', localData.recoveryInfo || "", "text", t.recoveryInfoHint)}
          ...
          {localData.type === 'contact' && (
            <>
              ...
              {copyableField(t.phone, 'phone', localData.phone || "", "number", t.phoneHint)}
              ...
              {localData.documentType && (
                <div className="space-y-5 animate-in fade-in w-full">
                  ...
                  <div className="space-y-4 w-full">
                    {localData.documentType === 'health_insurance' ? copyableField(t.insuranceId, 'idNumber', localData.idNumber || "", "number", "...") :
                     localData.documentType === 'driving_license' ? copyableField(t.licenseId, 'idNumber', localData.idNumber || "", "number", "...") :
                     localData.documentType === 'passport' ? copyableField(t.passportId, 'idNumber', localData.idNumber || "", "number", "...") :
                     copyableField("Số định danh", 'idNumber', localData.idNumber || "", "number", "Số hiệu định danh...", true, localData.documentType === 'id_card' ? () => setOcrModal({type: 'document', field: 'idNumber'}) : undefined)}
                    
                    {copyableField(localData.documentType === 'id_card' ? "Họ, chữ đệm và tên khai sinh" : t.fullName, 'fullName', localData.fullName || "", "text", "Họ và tên in hoa...", true, undefined, localData.documentType === 'id_card')}
                    
                    {copyableField("Ngày sinh", 'dob', localData.dob || "", "date", "")}
                    {localData.documentType === 'id_card' && copyableField("Có giá trị đến", 'expiryDate', localData.expiryDate || "", "date", "")}
                    ...
                    {localData.documentType === 'id_card' && copyableField("Quốc tịch", 'nationality', localData.nationality || "Việt Nam", "text", "")}
                  </div>
                  ...
                  {localData.documentType === 'id_card' && (
                    <div className="space-y-4 w-full">
                      ...
                      {copyableField("Ngày tháng năm cấp", 'issueDate', localData.issueDate || "", "date", "")}
                      {copyableField("Nơi cấp", 'issuePlace', localData.issuePlace || "", "text", "...")}
                    </div>
                  )}
      ...
      {ocrModal && (
        <div className="fixed inset-0 z-[200] bg-black/80 flex flex-col items-center justify-center p-6 backdrop-blur-md">
          <div className="w-full max-w-sm bg-[#121212] border border-white/10 rounded-[2.5rem] p-8 space-y-6 relative overflow-hidden">
            <div className="flex flex-col items-center text-center space-y-4">
              <div className="w-20 h-20 bg-[#4CAF50]/20 rounded-full flex items-center justify-center relative">
                <Icons.Camera className="text-[#4CAF50]" size={32} />
                {isOcrProcessing && (
                  <div className="absolute inset-0 border-4 border-[#4CAF50] border-t-transparent rounded-full animate-spin" />
                )}
              </div>
              <h3 className="text-lg font-bold text-white uppercase tracking-tight">Quét {ocrModal.type === 'card' ? 'Thẻ' : 'Căn cước'}</h3>
              <p className="text-gray-400 text-xs">Vui lòng chọn ảnh để trích xuất thông tin tự động.</p>
            </div>

            {!isOcrProcessing ? (
              <div className="space-y-3">
                <label className="w-full bg-[#4CAF50] text-white py-4 rounded-3xl font-bold text-center block cursor-pointer active:scale-95 transition-all shadow-lg shadow-[#4CAF50]/20">
                  CHỌN ẢNH TỪ THƯ VIỆN
                  <input type="file" className="hidden" accept="image/*" onChange={handleOcrScan} />
                </label>
                <button onClick={() => setOcrModal(null)} className="w-full py-4 text-gray-500 font-bold uppercase text-[10px] tracking-widest">HỦY BỎ</button>
              </div>
            ) : (
              <div className="py-8 flex flex-col items-center space-y-4">
                <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden relative">
                  <div className="absolute inset-0 bg-[#4CAF50] animate-[shimmer_2s_infinite]" style={{ width: '50%' }} />
                </div>
                <p className="text-[10px] font-black text-[#4CAF50] uppercase tracking-widest animate-pulse">Đang xử lý dữ liệu...</p>
              </div>
            )}
            
            {isOcrProcessing && (
              <div className="absolute inset-x-0 top-0 h-1 bg-[#4CAF50]/50 shadow-[0_0_15px_#4CAF50] animate-[scan_2s_linear_infinite]" />
            )}
          </div>
        </div>
      )}
...
const GeneratorScreen = ({ t, isDark, genPass, genConfig, setGenConfig, handleGenerator, copy, genHistory, showGenHistory, setShowGenHistory, setToast, onSave }: any) => {
  ...
  const [linkQrGenerated, setLinkQrGenerated] = useState(false);

  useEffect(() => {
    if (linkQrGenerated) {
      const timer = setTimeout(() => {
        setLinkUrl('');
        setLinkQrGenerated(false);
        setToast("QR Link đã hết hạn (5 phút)");
      }, 5 * 60 * 1000);
      return () => clearTimeout(timer);
    }
  }, [linkQrGenerated]);
  ...
  const handlePasteLink = async () => {
    try {
      ...
      setToast("Không thể truy cập bộ nhớ tạm");
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      ...
      <div className={`p-2 flex gap-1 border-b transition-colors ${isDark ? 'bg-black/20 border-white/5' : 'bg-gray-100 border-gray-200'}`}>
        {['password', 'wifi', 'share', 'link'].map((m: any) => (<button key={m} onClick={() => setGenMode(m)} className={`flex-1 py-2 text-[10px] font-black uppercase rounded-xl transition-all ${genMode === m ? 'bg-[#4CAF50] text-white shadow-lg' : 'text-gray-500'}`}>{t[`genMode${m.charAt(0).toUpperCase() + m.slice(1)}` as keyof typeof t]}</button>))}
      </div>
            ...
            <div className="space-y-6 max-w-lg mx-auto">
              <div className="space-y-4">
                <div className="relative">
                  <input 
                    value={linkUrl} 
                    onChange={e => { setLinkUrl(e.target.value); setLinkQrGenerated(false); }} 
                    className={`w-full border rounded-2xl py-4 px-5 pr-12 text-[15px] outline-none transition-all ${isDark ? 'bg-[#181818] border-white/5 text-white focus:border-[#4CAF50]/50' : 'bg-white border-gray-200 shadow-sm focus:border-[#4CAF50]/50'}`} 
                    placeholder="Nhập liên kết (URL)..." 
                  />
                  <button onClick={handlePasteLink} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#4CAF50] p-2 hover:bg-[#4CAF50]/10 rounded-full transition-all">
                    <Icons.Copy size={18} />
                  </button>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setLinkQrGenerated(true)} disabled={!linkUrl} className="bg-[#4CAF50] text-white py-4 rounded-3xl font-bold text-[11px] uppercase shadow-lg disabled:opacity-30 active:scale-95 transition-all flex items-center justify-center gap-2"><Icons.Camera size={18}/> {t.createQRCode}</button>
                  <button onClick={() => { setLinkUrl(''); setLinkQrGenerated(false); }} className={`py-4 rounded-3xl font-bold text-[11px] uppercase border active:scale-95 transition-all flex items-center justify-center gap-2 ${isDark ? 'bg-white/5 border-white/5 text-white' : 'bg-white border-gray-200 shadow-sm'}`}><Icons.Trash2 size={18}/> Xóa</button>
                </div>
              </div>
              {linkQrGenerated && linkUrl && (
